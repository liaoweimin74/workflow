...
- SearchTable 查询栏组件化（commit 待定）：单选选项类字段（select/tree/elTreeSelect/cascader 单选）查询栏生成 select 下拉组件（`resolveOptionRules` 解析选项，查询值=显示值 label，`_text` 列精确等值）；日期字段 date-picker；多选/文本 input 模糊 LIKE；后端无需改（`_text` VARCHAR 等值/主列 LIKE/日期等值已支持）；BizDataListPage 14/14（新增 3 用例）；前端全量 73 文件 926 测试全过，vue-tsc 无新增错误（仅既有 50/343）
- 页面设计器表格选项组件可查询（commit 待定）：①ViewDesigner `filterableColumns` 选项类组件主列（JSON，componentType select/multiSelect/elTransfer/tree/elTreeSelect/cascader）视为可筛（显示列页签"查询"勾选不再禁用）；②PageDataTable `fetchApi` 搜索栏对数组值组件主列映射 `<key>_text` 列查询（`resolveSearchColumn` 查 metaColumns 的 componentType；查询值=显示值 label，LIKE/等值匹配显示列）；③**PageDesigner 数据表格配置数据源窗口（DsBindingConfigDialog）`loadTableCandidates` 同因不可查询——`tableFilterableKeys` 同加选项类组件判定（checkbox/multiSelect/elTransfer/tree/elTreeSelect/cascader/select）**；后端无需改（`_text` LIKE/等值已支持）；ViewDesigner.test + PageDataTable.test + DsBindingConfigDialog.table.test 各增用例（相关 23 测试全过）；前端全量 73 文件 928 测试全过，vue-tsc 无新增错误（仅既有）
- 页面设计器表格编辑弹窗按表单 schema 构建组件（commit 待定）：①后端 `DataSourceMetadata` 新增 `formKey`（FORM/WORKFLOW metadata 返回；SYSTEM/API 空）；②PageDataTable `loadMetadata` 读 meta.formKey → `loadFormSchema`（formApi.getFormDefinitionByKey）加载业务表单 schema → `formRules` 用 schema（select/级联/树形/穿梭框 + 选项 + 校验按定义，无 `<key>_text` 重复列），SYSTEM/API 无 formKey 回退列定义映射；③createApi/updateApi 提交 `withArrayLabels(data, schema)`（生成 `_text` 显示列）；④`inputTypeOf` 补 `INT`（后端 columnType 实际值，原只认 INTEGER/BIGINT 致 INT 列生成 input 非 inputNumber——既有 bug）；⑤**FormRenderer `dsBindings` 空时回退全局 activeDsBindings** + ⑥**PageDataTable 保存业务表单 `schema.dataSources`（表单内 id → refId）经 `formConfig.dataSources` 与 3 处 FormRenderer `data-sources` 透传**（合修页面新增弹窗 select 选项数据源 effect.datasource 用表单内 id `ds_xxx` 查询全局数据源报"数据源不存在 404"——表单 schema 自带绑定映射）；⑦**resolvedColumns 用户配置列数组值组件（metaColumns 查 componentType）覆盖 render 优先读 `<key>_text`（leafDisplayText 叶子 label），缺失回退 value join**（页面列表显示显示值而非原始 value，与 BizDataListPage 一致）；UnifiedDataSourceAdapterTest 加 formKey 断言（22 测试全过）+ PageDataTable.test 加 schema 构建/dataSources/数组值显示断言（15 测试全过）+ FormRenderer.test；前端全量 73 文件 931 测试全过，后端 821 仅 1 既有失败，vue-tsc 无新增错误；**用户浏览器验证 page2 新增弹窗组件正常 + 列表显示显示值，确认修复**

- 查询组件按字段组件类型构建（commit 待定）：①`SearchField` 类型扩展 `cascader`/`lookupPicker`，`SearchTable` 模板加 el-cascader 与 LookupPicker（查询输入=字段组件类型：选择器/多选框/穿梭框→select 下拉；树形→tree-select；级联→cascader；数据引用 lookupPicker/dataPicker→LookupPicker；日期→date-picker；其余→input）；②BizDataListPage/PageDataTable 查询组件按 schema rule/metaColumns componentType 生成；③**选项数据源取数**：PageDataTable `loadFormSchema` 加 `resolveOptionRules`（effect.datasource → 选项列表，按表单字段配置取数）、穿梭框选项从 `props.data` 取（optionTarget 承载字段）；④查询匹配结构化 filter：选项类/文本 LIKE、数值/日期等值（级联路径 label 数组 join('/')）；⑤**PageDataCards 数组值列 formatter 读 `<key>_text`（leafDisplayText 叶子 label）回退 value join**（卡片列表显示显示值，对齐 PageDataTable；运行态加载 metadata 查 componentType）；BizDataListPage.test（多选框/穿梭框 select+选项）+ PageDataTable.test（select/tree-select/cascader 组件 + 选项 + _text 映射）+ PageDataCards.test（数组值 formatter）；前端全量 73 文件 933 测试全过，vue-tsc 无新增错误

- 首次访问表格/卡片列表数组值列显示原始值修复（commit 32d5ec5）：根因——SearchTable/ListCards（子组件）`onMounted` 立即取数，早于父组件 PageDataTable/PageDataCards 的 `loadMetadata()`（getMetadata 网络异步）完成，首次取数时 `metaColumns` 为空 → 列无 formatter → 渲染原始 value；metaColumns 到达后列定义更新但 element-plus 已渲染行不因列 render 变化重算单元格 → 再点查询（重新取数用最新列定义）才显示 label。修复：加 `metaLoaded` 标记，`loadMetadata()` 完成后才挂载 SearchTable/ListCards（`v-if="metaLoaded"`），子组件挂载取数时列定义已就绪；PageDataCards 数据源切换 watch 置 metaLoaded=false 卸载重挂（删并发 fetchData）。另含：ListCards 查询栏按字段组件类型生成（select/tree-select/cascader/lookupPicker/date-picker）、空状态去重试按钮（错误状态保留）、PageDataCards 表单/查询参照 PageDataTable（formRules/data-sources/查询映射 `_text`）、`DataSourceMetadataDTO` 补 `formKey` 字段；PageDataTable.test + PageDataCards.test + ListCards.test 相关 89 测试全过；前端全量 73 文件 933 测试全过，vue-tsc 无新增错误（仅既有）；**用户浏览器验证刷新后首次访问 page2 表格全部显示 label（test_guest/武汉分公司 等），确认修复**
