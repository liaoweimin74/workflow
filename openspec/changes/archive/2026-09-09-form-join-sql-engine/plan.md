...
**Goal:** 为 FORM 数据源引入跨表 JOIN 关联查询，config（声明式 JOIN）与 sql（SQL 模板）双模式并存，经统一 SQL 执行引擎实现关联字段展示/排序/筛选，并支持 sql 模式运行时参数从前端透传（如统计时间段），前端零改动。

**Architecture:** 在 DataSourceAdapter SPI 链路内，FORM 数据源 query 按 params.queryMode 分流：config 模式经 JoinSqlGenerator 生成 JOIN SQL，sql 模式经 SqlTemplateEngine 将管理员 SQL 包裹为子查询；两模式产出统一 SqlAndParams 交共享执行器（复用 BizDataQueryBuilder 的分页/排序/筛选/租户隔离逻辑）。sql 模式支持 params 白名单，前端经查询请求 `params` 段透传值并参数化绑定到 `:paramName`。metadata 增加虚拟列（config 推导 / sql 从 columns 映射），filter/sort 白名单扩展覆盖虚拟列。
...
## Task 3a: SQL 模板运行时参数透传

- [ ] **Step 1:** `BizDataQueryRequest` 增加 `params`（JSON 字符串）字段，支持前端透传自定义查询参数（与 filter 区分：filter 是列筛选，params 是 SQL 占位符绑定）。
- [ ] **Step 2:** sql 模式配置增加 `params` 白名单声明（如 startTime/endTime），保存校验参数名合法且与 SQL 占位符匹配。
- [ ] **Step 3:** SqlTemplateEngine 将命中白名单的前端参数值参数化绑定到 `:paramName`；未声明/非法键拒绝（400）。
- [ ] **Step 4:** 单测声明参数绑定、未声明参数拒绝、无 params 段缺省执行。
...
- [ ] **Step 4:** 集成测试 FORM 数据源 config/sql 的 metadata 与 query 端到端（关联排序/筛选/分页/参数透传）。

## Task 5: FORM 数据源 params 开放 JOIN/SQL 编辑（后端）

- [ ] **Step 1:** DataSourceDefinitionService 支持 FORM 查询配置段（含 params 白名单）校验与保存，CRUD 接口 params 段保持不变。
...
- [ ] **Step 3:** sql 模式 UI（query 文本域 + columns 声明编辑 + params 白名单声明编辑）。
- [ ] **Step 4:** 保存提交 queryMode + 配置；查询侧零改动验证虚拟列渲染与排序/筛选。
- [ ] **Step 5:** 前端查询请求类型（BizDataQueryParams）增加 `params` 透传。
