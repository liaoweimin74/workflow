...
describe('节点表单配置 — 字段数据来源映射', () => {
  ...
  it('权限下拉提供「编辑 / 只读 / 隐藏」选项（文案调整）', async () => {
    setupNodeConfig()
    const wrapper = await mountTab()

    const row = wrapper.findAll('.el-table__row').find((r: any) => r.text().includes('金额'))!
    const permSelect = row.find('.perm-select .el-select__wrapper')
    expect(permSelect.exists()).toBe(true)
    await permSelect.trigger('click')
    await flushPromises()
