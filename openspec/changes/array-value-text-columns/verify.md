...
- 页面设计器表格选项组件可查询（commit 待定）：①ViewDesigner `filterableColumns` 选项类组件主列（JSON，componentType select/multiSelect/elTransfer/tree/elTreeSelect/cascader）视为可筛（显示列页签"查询"勾选不再禁用）；②PageDataTable `fetchApi` 搜索栏对数组值组件主列映射 `<key>_text` 列查询（`resolveSearchColumn` 查 metaColumns 的 componentType；查询值=显示值 label，LIKE/等值匹配显示列）；后端无需改（`_text` LIKE/等值已支持）；ViewDesigner.test（filterableKeys 含 select 列）+ PageDataTable.test（dept → dept_text 查询断言）各增用例，相关 16 测试全过；前端全量 73 文件 927 测试全过，vue-tsc 无新增错误（仅既有 13/311 等）
