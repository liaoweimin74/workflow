...
import type { ReferencePickerProps, QueryParams } from './types'
...
const displayText = computed(() => {
  ...
  if (Array.isArray(props.modelValue)) {
    return props.modelValue
      .map((item: any) => {
        if (typeof item === 'object') return item[props.displayField] || ''
        return item
      })
      .join(' / ')
  }
  const found = tableData.value.find(
    (row) => row[props.valueField] === props.modelValue,
  )
  return found ? found[props.displayField] : String(props.modelValue)
})
...
function confirmSelection() {
  const values = tempSelection.value.map((row) => row[props.valueField])
