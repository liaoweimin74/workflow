...
### Requirement: 对话窗体与品牌样式

悬浮球点击后 SHALL 打开**完全悬浮的对话窗体**（固定浮层，非侧边抽屉），再次点击悬浮球 SHALL 收起。窗体 SHALL 采用**深色标题栏**以突出存在感，并以机器人图标表达 AI 形象：

- 标题 SHALL 显示「小智·AI 助手」，标题栏含机器人图标；
- 清除历史 SHALL 以**图标按钮**提供（历史为空时禁用）；
- 助手消息 SHALL 在气泡前显示机器人头像。

窗体 SHALL 保留多轮对话历史（前端会话内维护，不上后端、不落库）；清空 SHALL 移除全部历史消息。输入框 SHALL 默认 1 行，随内容自动增高，**最多 5 行**，超出后显示滚动条。

#### Scenario: 打开与收起

- **WHEN** 用户点击右下角悬浮球
- **THEN** 打开悬浮对话窗体（深色标题栏 + 机器人图标 + 「小智·AI 助手」）
- **AND** 再次点击悬浮球收起

#### Scenario: 图标化清除历史

- **WHEN** 存在对话历史时点击标题栏的清除图标
- **THEN** 历史消息全部移除
- **AND** 无历史时该图标禁用
