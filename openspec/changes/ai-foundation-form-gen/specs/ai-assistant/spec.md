...
助手 SHALL 能在回复中提供可点击的页面入口，点击后跳转到对应界面。系统 SHALL 提供 `open_page` 工具，其 `path` 参数 SHALL 仅接受**当前用户可访问页面白名单**中的路径，不接受任意 URL。白名单 SHALL 由客户端随对话请求提供，取值为**用户菜单树展平出的全部页面**（含所有菜单）。工具结果 SHALL 作为助手消息的页面入口，以**文字 tag 链接**呈现；点击 SHALL 通过前端路由跳转；若已在目标页 SHALL 不重复跳转。

#### Scenario: 提供页面入口并跳转

- **WHEN** 助手需引导用户前往某页面（如流程定义）
- **THEN** 调用 open_page 并提供白名单内的 path
- **AND** 助手消息展示可点击的文字 tag 入口
- **AND** 点击后路由跳转到该页面
